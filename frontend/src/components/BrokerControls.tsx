  ...
  return (
    <section className="panel controls-panel">
      <div className="section-heading compact">
        ...
        <h2>Recovery and external clients</h2>
        <p>Trigger broker recovery manually or copy examples for real producer and consumer services.</p>
      </div>

      <button className="recover-button" disabled={isDisabled} onClick={onRecover} type="button">
        Recover delayed and expired messages
      </button>

      <div className="curl-grid">
        <CurlBlock title="Producer publish" command={publishCurl} />
        <CurlBlock title="Consumer reserve" command={consumeCurl} />
        <CurlBlock title="Consumer ack" command={ackCurl} />
        <CurlBlock title="Consumer nack" command={nackCurl} />
      </div>
    </section>
  );
}
